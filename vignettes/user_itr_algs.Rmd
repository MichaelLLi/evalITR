...
It should also specify the way to get the ITR, based on the estimated effects. In the following 
example, we fit a linear model with sample splitting and use the estimated CATE. We compute the ITR by
...
It do not have to be a linear model or model that estimate CATE. In addition, the scoring rule can be
any rule that assigns treatment based on the score and map the score to a binary indicator of treatment assignment.
